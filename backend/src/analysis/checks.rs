...
pub fn check_low_temperature(
    ...
    let mut flush = |group: &mut Vec<&Sample<f64>>| {
        ...
        out.push(mk(
            ...
            json!({"min_c": min_v, "limit_c": min_temp_c, "samples": group.len()}),
        ));
        group.clear();
    };
    for s in temps {
...
pub fn check_clock_drift(
    ...
    let mut scan: Vec<(String, DateTime<Utc>, i64)> = Vec::new();
    let mut collect = |skew_ms: i64, at: DateTime<Utc>, src: &str| {
        if skew_ms.abs() > limit_ms {
            scan.push((src.to_string(), at, skew_ms));
        }
    };
    for s in temps {
        collect(s.skew().num_milliseconds(), s.device_time, &s.source);
    }
    for s in flows {
        collect(s.skew().num_milliseconds(), s.device_time, &s.source);
    }
    ...
    // 按通道聚合
    let mut sources: Vec<&str> = scan.iter().map(|(s, _, _)| s.as_str()).collect();
    sources.sort();
    sources.dedup();
    for src in sources {
        let mut rows: Vec<_> = scan.iter().filter(|(s, _, _)| s.as_str() == src).collect();
        rows.sort_by_key(|(_, _, k)| -k.abs());
        let worst = rows[0];
        out.push(mk(
            FindingKind::ClockDrift,
            Severity::Warning,
            Some((worst.1, worst.1)),
            format!(
                "通道 {src} 时钟差超阈:最差 {:.1}s(阈值 {:.1}s),共 {} 个样本",
                worst.2 as f64 / 1000.0,
                spec.max_clock_skew_s,
                rows.len()
            ),
            json!({"source": src, "worst_skew_s": worst.2 as f64 / 1000.0, "samples": rows.len()}),
        ));
    }
    out
}
...
pub fn check_evidence_gaps(
    ...
    let gaps_of = |series: &[Sample<f64>], name: &str, out: &mut Vec<Finding>| {
            ...
            if let Some(p) = prev {
                if s.device_time - p > limit {
                    out.push(mk(
                        ...
                        json!({"channel": name, "gap_s": (s.device_time - p).num_milliseconds() as f64 / 1000.0}),
                    ));
                }
            }
        ...
        if let Some(p) = prev {
            if end - p > limit {
                out.push(mk(
                    ...
                    json!({"channel": name, "gap_s": (end - p).num_milliseconds() as f64 / 1000.0}),
                ));
            }
        }
    };
    gaps_of(temps, "温度", &mut out);
    gaps_of(flows, "流量", &mut out);
    out
}
...
pub fn check_flow_drop(flows: &[Sample<f64>], ratio: f64) -> Vec<Finding> {
    ...
    let mut flush = |group: &mut Vec<&Sample<f64>>| {
        ...
        out.push(mk(
            ...
            json!({"min_lph": min_v, "baseline_lph": baseline, "threshold_lph": threshold}),
        ));
        group.clear();
    };
    for s in flows {
...
pub fn check_divert_feedback_delay(
    ...
    vec![mk(
        ...
        json!({"worst_s": worst.skew().num_milliseconds() as f64 / 1000.0, "late_samples": late.len()}),
    )]
}
        ...
        .filter(|n| n.kind == NoticeKind::ProbeRecalibrated)
    {
        ...
        match vs.iter().find(|v| v.calibrated_at >= n.reported_at) {
            None => out.push(mk(
                ...
                json!({"instrument_id": inst, "notice_at": n.reported_at}),
            )),
            Some(v) => {
                let prev = vs
                    .iter()
                ...
                if let Some(p) = prev {
                    if p.label == v.label {
                        out.push(mk(
                            ...
                            json!({"instrument_id": inst, "label": v.label, "notice_at": n.reported_at}),
                        ));
                    }
                }
            }
